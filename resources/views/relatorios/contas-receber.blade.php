...
<style>
    ...
    .table thead th { position: sticky; top: 0; background: #28a745; color: white; z-index: 10; }
...
<h1 class="no-print"><i class="fas fa-chart-line mr-2"></i>Relatório Contas a Receber</h1>
@stop
...
<div class="container-fluid">
    ...
    <div class="card no-print">
        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-md-2">
                    <label>Cliente</label>
                    <input type="text" class="form-control" id="filtroCliente" placeholder="Nome">
                </div>
                <div class="col-md-2">
    ...
    <div class="print-summary">
        <div class="print-summary-content">
            ...
            <div class="print-summary-item">
                <div class="label">Total Valor Recebido</div>
                <div class="value" id="printTotalRecebido">R$ 0,00</div>
            </div>
        </div>
    </div>
...
function renderizarTabela() {
    ...
    if (contasCarregadas.length === 0) {
        ...
        contasCarregadas.forEach(function(conta) {
            ...
            var valorRecebido = parseFloat(conta.valor_recebido || 0);
            var statusBadge = getStatusBadge(conta.status, valorRecebido, valorLiquido);
            
            html += '<tr>';
            html += '<td class="text-center">' + conta.id + '</td>';
            html += '<td>' + (conta.documento || conta.numero_documento || '-') + '</td>';
            html += '<td>' + (conta.descricao || '-') + '</td>';
            html += '<td>' + (conta.cliente_nome || conta.cliente || '-') + '</td>';
            html += '<td>' + (conta.centro_custo_nome || '-') + '</td>';
            html += '<td class="text-right">' + formatarMoeda(valorLiquido) + '</td>';
            html += '<td class="text-right">' + formatarMoeda(valorRecebido) + '</td>';
...
function atualizarDadosImpressao() {
    ...
    // Totais
    var totalLiquido = 0, totalRecebido = 0;
    contasCarregadas.forEach(function(conta) {
        totalLiquido += parseFloat(conta.valor_liquido || conta.valor || 0);
        totalRecebido += parseFloat(conta.valor_recebido || 0);
    });
    
    $('#printTotalRegistros').text(contasCarregadas.length);
    $('#printTotalLiquido').text(formatarMoeda(totalLiquido));
    $('#printTotalRecebido').text(formatarMoeda(totalRecebido));
}

function getStatusBadge(status, valorRecebido, valorLiquido) {
    if (valorRecebido > 0 && valorRecebido < valorLiquido) {
        return '<span class="badge badge-info">Parcial</span>';
    }
    
    switch(status) {
        case 'recebido': return '<span class="badge badge-success">Recebido</span>';
...
function exportarExcel() {
    ...
    window.location.href = '/relatorios/contas-receber/export?' + params.toString();
}
</script>
@stop
